...
import { submitTempoBooking } from '@/app/actions/tempo-booking'; // We will create this action
import { Loader2 } from 'lucide-react';

export function TempoBookingForm({ vehicleId, vehicleName }: { vehicleId: string, vehicleName: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await submitTempoBooking(formData);
      if (res.success) {
        setStatus('success');
  ...
  if (status === 'success') {
    return (
      <div className="bg-stone-900 border border-gold-500/30 p-8 text-center rounded-sm">
        ...
        <p className="text-stone-300">Thank you for your booking enquiry for the {vehicleName}. Our team will contact you shortly to confirm availability and exact pricing.</p>
