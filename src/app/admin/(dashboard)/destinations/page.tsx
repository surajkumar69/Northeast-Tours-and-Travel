      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Manage Meghalaya, Assam, Arunachal Pradesh and other locations.
          </p>
        </div>
                  ...
                  {destinations?.length === 0 ? (
                    ...
                    destinations?.map((dest) => (
                      <tr key={dest.id}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{dest.state}</td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${dest.is_published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                            {dest.is_published ? 'Published' : 'Draft'}
                          </span>
                        </td>
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <div className="flex justify-end gap-3">
                            <Link href={`/admin/destinations/${dest.id}/edit`} className="text-gray-600 hover:text-gray-900">
